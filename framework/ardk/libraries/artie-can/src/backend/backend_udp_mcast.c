...
#endif
{
    ...
    ARTIE_CAN_LOG(context, "[UDP MCAST] Receiver thread started\n");
    ...
    while (!mcast_ctx->should_stop)
    {
        ...
        int recv_size = recvfrom(mcast_ctx->socket_fd, recvbuf, sizeof(recvbuf), 0,
        ...
        if (recv_size > 0)
        {
            if (recv_size == sizeof(artie_can_frame_t))
            {
                ...
                ARTIE_CAN_LOG(context, "[UDP MCAST] Received frame (%d bytes)\n", recv_size);
                _complete_frame(context, recvbuf);
            }
            else
            {
                // Partial or malformed frame
                ARTIE_CAN_LOG(context, "[UDP MCAST] Received malformed frame (%d bytes, expected %zu)\n",
        ...
        else if (recv_size == SOCKET_ERROR_VALUE)
        {
            ...
            {
                ARTIE_CAN_LOG(context, "[UDP MCAST] Receive error: %d\n", get_socket_error());
            }
        }
    }

    ARTIE_CAN_LOG(context, "[UDP MCAST] Receiver thread exiting\n");
#ifdef _WIN32
...
static artie_can_error_t _init_udp_mcast(artie_can_context_t *context)
{
    ...
    {
        ARTIE_CAN_LOG(context, "[UDP MCAST] Failed to create socket\n");
        return ARTIE_CAN_ERR_INIT_FAIL;
    }
    ...
    {
        ARTIE_CAN_LOG(context, "[UDP MCAST] Failed to set SO_REUSEADDR\n");
    ...
    {
        ARTIE_CAN_LOG(context, "[UDP MCAST] Warning: Failed to set SO_REUSEPORT\n");
    ...
    {
        ARTIE_CAN_LOG(context, "[UDP MCAST] Warning: Failed to set receive timeout\n");
        // Non-fatal
    }
    ...
    if (bind(mcast_ctx->socket_fd, (struct sockaddr *)&local_addr, sizeof(local_addr)) == SOCKET_ERROR_VALUE)
    {
        ARTIE_CAN_LOG(context, "[UDP MCAST] Failed to bind socket to port %d\n", mcast_ctx->config.port);
        close_socket(mcast_ctx->socket_fd);
        return ARTIE_CAN_ERR_INIT_FAIL;
    }
    ...
    {
        ARTIE_CAN_LOG(context, "[UDP MCAST] Invalid multicast address: %s\n", mcast_ctx->config.group_addr);
        close_socket(mcast_ctx->socket_fd);
        return ARTIE_CAN_ERR_INIT_FAIL;
    }
    ...
    {
        ARTIE_CAN_LOG(context, "[UDP MCAST] Failed to join multicast group %s\n", mcast_ctx->config.group_addr);
        close_socket(mcast_ctx->socket_fd);
        return ARTIE_CAN_ERR_INIT_FAIL;
    }

    ARTIE_CAN_LOG(context, "[UDP MCAST] Joined multicast group %s:%d\n",
    ...
    {
        ARTIE_CAN_LOG(context, "[UDP MCAST] Failed to create receiver thread\n");
        close_socket(mcast_ctx->socket_fd);
        return ARTIE_CAN_ERR_INIT_FAIL;
    }
    ...
    ARTIE_CAN_LOG(context, "[UDP MCAST] UDP multicast backend initialized\n");
...
static artie_can_error_t _send_udp_mcast(void *ctx, const artie_can_frame_t *frame)
{
    ...
    {
        ARTIE_CAN_LOG(context, "[UDP MCAST] Invalid multicast address\n");
        return ARTIE_CAN_ERR_SEND_FAIL;
    }

    // Send the frame to the multicast group
    ARTIE_CAN_LOG(context, "[UDP MCAST] Sending frame with ID 0x%X to multicast group\n", frame->id);
    int send_result = sendto(mcast_ctx->socket_fd, (const char *)frame, sizeof(artie_can_frame_t), 0, (struct sockaddr *)&dest_addr, sizeof(dest_addr));

    if (send_result == SOCKET_ERROR_VALUE)
    {
        ARTIE_CAN_LOG(context, "[UDP MCAST] Failed to send frame\n");
        return ARTIE_CAN_ERR_SEND_FAIL;
    }
...
static artie_can_error_t _close_udp_mcast(artie_can_context_t *context)
{
    ...
    ARTIE_CAN_LOG(context, "[UDP MCAST] UDP multicast backend closed\n");
    return ARTIE_CAN_ERR_NONE;
}
...
artie_can_error_t udp_mcast_init(artie_can_context_t *context, artie_can_backend_t *handle, artie_can_rx_callback_t rx_callback, artie_can_get_ms_t get_ms_fn)
{
    ...
    // Store the callback
    mcast_ctx->rx_callback = rx_callback;

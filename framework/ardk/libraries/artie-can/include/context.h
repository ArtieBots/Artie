...
typedef artie_can_error_t driver_isr_handler_t(artie_can_context_t *context);
...
typedef struct {
    ...
    driver_isr_handler_t *isr_handler;      ///< The function that the microcontroller should call when an interrupt is generated by the CAN peripheral. This is populated by the backend.
    artie_can_rx_callback_t *rx_callback;   ///< Callback function for received CAN frames
    uint8_t node_address;                   ///< The address of this node on the CAN bus, according to the protocols that use it.
} artie_can_context_t;

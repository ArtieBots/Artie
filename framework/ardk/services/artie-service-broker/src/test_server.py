"""
A unittesting server for the Artie Service Broker service.
"""
...
class TestService(
    ...
    def __init__(self, port: int):
        ...
        alog.info(f"Test service started on port {port}.")
...
def start_test_service_broker_server(broker_port: int):
    """
    Start the test service broker server on a random port,
    ...
    os.environ[constants.ArtieEnvVariables.ARTIE_SERVICE_BROKER_PORT] = str(broker_port)
    ...
    # Wait for the service broker to come online
    alog.info(f"Waiting for service broker to come online on port {broker_port}...")
    while not _port_in_use(broker_port):
        time.sleep(1)

    # Instantiate the single server instance and block forever, serving
    test_service = TestService(port)
    t = util.create_server(test_service, keyfpath, certfpath, port)
    t.start()
